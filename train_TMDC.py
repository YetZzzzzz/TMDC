...
import sys
from utils_TMDC import Logger, get_loaders, build_model, generate_mask, generate_inputs
...
def train_or_eval_model(args, model, reg_loss, cls_loss, dataloader, optimizer=None, train=False, first_stage=True, mark='train'):
    ...
    preds_a_common, preds_t_common, preds_v_common, = [], [], []
    preds_intra_a, preds_intra_t, preds_intra_v, preds_inter_a, preds_inter_t, preds_inter_v,  = [], [], [], [], [], []
    ...
    for data_idx, data in enumerate(dataloader):
        ...
        '''
        ## forward
        hidden, out, outputs_intras, outputs_inters, vib_outputs, vib_kls = model(masked_input_features[0], input_features_mask[0], umask, first_stage)
        in_mask = torch.clone(input_features_mask[0].permute(1, 0, 2))
        in_mask[umask == 0] = 0
        outputs_intra_a, outputs_intra_t, outputs_intra_v = outputs_intras[0], outputs_intras[1], outputs_intras[2]
        outputs_inter_a, outputs_inter_t, outputs_inter_v = outputs_inters[0], outputs_inters[1], outputs_inters[2]
        out_a, out_t, out_v, out_a_common, out_t_common, out_v_common = vib_outputs[0], vib_outputs[1], vib_outputs[2], vib_outputs[3], vib_outputs[4], vib_outputs[5]
        a_kls, t_kls, v_kls, a_kls_common, t_kls_common, v_kls_common = vib_kls[0], vib_kls[1], vib_kls[2], vib_kls[3], vib_kls[4], vib_kls[5]
        
        ## calculate loss
        lp_ = out.view(-1, out.size(2)) # [batch*seq_len, n_classes]
        # intra_loss
        lp_intra_a, lp_intra_t, lp_intra_v = outputs_intra_a.view(-1, outputs_intra_a.size(2)), outputs_intra_t.view(-1, outputs_intra_t.size(2)), outputs_intra_v.view(-1, outputs_intra_v.size(2))
        # inter_loss
        lp_inter_a, lp_inter_t, lp_inter_v = outputs_inter_a.view(-1, outputs_inter_a.size(2)), outputs_inter_t.view(-1, outputs_inter_t.size(2)), outputs_inter_v.view(-1, outputs_inter_v.size(2))
        # vib_loss
        lp_a, lp_t, lp_v = out_a.view(-1, out_a.size(2)), out_t.view(-1, out_t.size(2)), out_v.view(-1, out_v.size(2))
        lp_a_common, lp_t_common, lp_v_common = out_a_common.view(-1, out_a_common.size(2)), out_t_common.view(-1, out_t_common.size(2)), out_v_common.view(-1, out_v_common.size(2))
        labels_ = label.view(-1) # [batch*seq_len]
        if dataset in ['IEMOCAPFour', 'IEMOCAPSix']:
            if first_stage:
                loss_intra_a = cls_loss(lp_intra_a, labels_, umask)
                loss_intra_t = cls_loss(lp_intra_t, labels_, umask)
                loss_intra_v = cls_loss(lp_intra_v, labels_, umask)
                loss_inter_a = cls_loss(lp_inter_a, labels_, umask)
                loss_inter_t = cls_loss(lp_inter_t, labels_, umask)
                loss_inter_v = cls_loss(lp_inter_v, labels_, umask)
                loss_a = cls_loss(lp_a, labels_, umask) + a_kls* 0.01
                loss_t = cls_loss(lp_t, labels_, umask) + t_kls* 0.01
                loss_v = cls_loss(lp_v, labels_, umask) + v_kls* 0.01
                loss_a_common = cls_loss(lp_a_common, labels_, umask) + a_kls_common* 0.01
                loss_t_common = cls_loss(lp_t_common, labels_, umask) + t_kls_common* 0.01
                loss_v_common = cls_loss(lp_v_common, labels_, umask) + v_kls_common* 0.01
            else:
                loss = cls_loss(lp_, labels_, umask)
        if dataset in ['CMUMOSI', 'CMUMOSEI']:
            if first_stage:
                loss_intra_a = reg_loss(lp_intra_a, labels_, umask)
                loss_intra_t = reg_loss(lp_intra_t, labels_, umask)
                loss_intra_v = reg_loss(lp_intra_v, labels_, umask)
                loss_inter_a = reg_loss(lp_inter_a, labels_, umask)
                loss_inter_t = reg_loss(lp_inter_t, labels_, umask)
                loss_inter_v = reg_loss(lp_inter_v, labels_, umask)
                loss_a = reg_loss(lp_a, labels_, umask) + a_kls* 0.01
                loss_t = reg_loss(lp_t, labels_, umask) + t_kls* 0.01
                loss_v = reg_loss(lp_v, labels_, umask) + v_kls* 0.01
                loss_a_common = reg_loss(lp_a_common, labels_, umask) + a_kls_common* 0.01
                loss_t_common = reg_loss(lp_t_common, labels_, umask) + t_kls_common* 0.01
                loss_v_common = reg_loss(lp_v_common, labels_, umask) + v_kls_common* 0.01
            else:
                loss = reg_loss(lp_, labels_, umask)

        # intra-inter results
        preds_a.append(lp_intra_a.data.cpu().numpy())
        preds_t.append(lp_intra_t.data.cpu().numpy())
        preds_v.append(lp_intra_v.data.cpu().numpy())
        preds_a_common.append(lp_inter_a.data.cpu().numpy())
        preds_t_common.append(lp_inter_t.data.cpu().numpy())
        preds_v_common.append(lp_inter_v.data.cpu().numpy())
        ...
        if train and first_stage:
            loss_as = loss_a + loss_a_common + loss_intra_a + loss_inter_a
            loss_ts = loss_t + loss_t_common + loss_intra_t + loss_inter_t
            loss_vs = loss_v + loss_v_common + loss_intra_v + loss_inter_v
            loss_as.backward()
            loss_ts.backward()
            loss_vs.backward()
            optimizer.step()
    ...
    preds_a_common = np.concatenate(preds_a_common)
    preds_t_common = np.concatenate(preds_t_common)
    preds_v_common = np.concatenate(preds_v_common)
    ...
    if dataset in ['IEMOCAPFour', 'IEMOCAPSix']:
        ...
        preds_a_common = np.argmax(preds_a_common, 1)
        preds_t_common = np.argmax(preds_t_common, 1)
        preds_v_common = np.argmax(preds_v_common, 1)
        ...
        avg_acc_a_common = accuracy_score(labels, preds_a_common, sample_weight=masks)
        avg_acc_t_common = accuracy_score(labels, preds_t_common, sample_weight=masks)
        avg_acc_v_common = accuracy_score(labels, preds_v_common, sample_weight=masks)
        return mae, ua, avg_accuracy, avg_fscore, [avg_acc_a, avg_acc_t, avg_acc_v, avg_acc_a_common, avg_acc_t_common, avg_acc_v_common], vidnames, avg_loss
        ...
        avg_acc_a_common = accuracy_score((labels[non_zeros] > 0), (preds_a_common[non_zeros] > 0))
        avg_acc_t_common = accuracy_score((labels[non_zeros] > 0), (preds_t_common[non_zeros] > 0))
        avg_acc_v_common = accuracy_score((labels[non_zeros] > 0), (preds_v_common[non_zeros] > 0))
        return mae, corr, avg_accuracy, avg_fscore, [avg_acc_a, avg_acc_t, avg_acc_v, avg_acc_a_common, avg_acc_t_common, avg_acc_v_common], vidnames, avg_loss
...
if __name__ == '__main__':
    ...
    for ii in range(args.num_folder):
        ...
        train_acc_as_common, train_acc_ts_common, train_acc_vs_common = [], [], []

        test_acc_as, test_acc_ts, test_acc_vs = [], [], []
        test_acc_as_common, test_acc_ts_common, test_acc_vs_common = [], [], []
        ...
        for epoch in range(args.epochs):
            ...
            train_mae, train_corr, train_acc, train_fscore, train_acc_atv, train_names, train_loss = train_or_eval_model(args, model, reg_loss, cls_loss, train_loader, \
                                                                            optimizer=optimizer, train=True, first_stage=first_stage, mark='train')
            test_mae, test_corr, test_acc, test_fscore, test_acc_atv, test_names, test_loss = train_or_eval_model(args, model, reg_loss, cls_loss, test_loader, \
            ...
            train_acc_as_common.append(train_acc_atv[3])
            train_acc_ts_common.append(train_acc_atv[4])
            train_acc_vs_common.append(train_acc_atv[5])
            test_acc_as.append(test_acc_atv[0])
            test_acc_ts.append(test_acc_atv[1])
            test_acc_vs.append(test_acc_atv[2])
            test_acc_as_common.append(test_acc_atv[3])
            test_acc_ts_common.append(test_acc_atv[4])
            test_acc_vs_common.append(test_acc_atv[5])

            if first_stage:
                print(f'epoch:{epoch}; a_acc_train:{train_acc_atv[0]:.3f}; t_acc_train:{train_acc_atv[1]:.3f}; v_acc_train:{train_acc_atv[2]:.3f}; common_a_acc_train:{train_acc_atv[3]:.3f}; common_t_acc_train:{train_acc_atv[4]:.3f}; common_v_acc_train:{train_acc_atv[5]:.3f}')
                print(f'epoch:{epoch}; a_acc_test:{test_acc_atv[0]:.3f}; t_acc_test:{test_acc_atv[1]:.3f}; v_acc_test:{test_acc_atv[2]:.3f}; common_a_acc_test:{test_acc_atv[3]:.3f}; common_t_acc_test:{test_acc_atv[4]:.3f}; common_v_acc_test:{test_acc_atv[5]:.3f}')
            else:
            ...
            if epoch == args.stage_epoch-1:
                ...
                model_idx_a = int(torch.argmax(torch.Tensor(test_acc_as)))
                print(f'best_epoch_a: {model_idx_a}')
                model_a = models[model_idx_a]
                models_a_para_dict = {k: v for k, v in model_a.state_dict().items() if 'intra_attn_as' in k}
                # models_a_para_dict2 = {k: v for k, v in model_a.state_dict().items() if 'Transformer' in k}
                model.state_dict().update(models_a_para_dict)

                model_idx_t = int(torch.argmax(torch.Tensor(test_acc_ts)))
                print(f'best_epoch_t: {model_idx_t}')
                model_t = models[model_idx_t]
                models_t_para_dict = {k: v for k, v in model_t.state_dict().items() if 'intra_attn_ts' in k}
                model.state_dict().update(models_t_para_dict)

                model_idx_v = int(torch.argmax(torch.Tensor(test_acc_vs)))
                print(f'best_epoch_v: {model_idx_v}')
                model_v = models[model_idx_v]
                models_v_para_dict = {k: v for k, v in model_v.state_dict().items() if 'intra_attn_vs' in k}
                model.state_dict().update(models_v_para_dict)
